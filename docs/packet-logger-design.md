...
- **The UF2 must match the board.** The RAK4631 is an nRF52840; the RAK11310 is an RP2040 — a different processor on a similarly named WisBlock core, listed beside it on the same download page. A mismatched UF2 is not refused loudly. The bootloader silently declines it and **leaves the file sitting on the mass-storage volume**, which looks exactly like a drag-and-drop that did not take, and invites people to try again with the same wrong file. Check the filename says `rak4631` before copying it — and better, check the image rather than its name. The filename is a label; the family ID inside every block is what the bootloader actually matches on. `firmware/radio/check_uf2.py` reads it: a RAK4631 image reports `0xADA52840` and loads at `0x00026000`, a RAK11310 image reports `0xE48BFF56` and loads at `0x10000000`.
- **There is more than one image called `rak4631`.** The nrf52840 release zip contains four: the plain build, `_eink`, `_eth_gw` and `_nomadstar_meteor_pro`. Only the plain one is pinned, and a variant flashes and boots perfectly, so nothing afterwards will tell you which one went on. The cache in `firmware/radio/` holds the pinned image and nothing else, so there is nothing to pick wrongly from.
...
| **R** Radio | Still booting; reset, brownout, connector shaken loose | Any word from the radio counts as proof of life. At 6 minutes of silence it is asked directly every 30 s; at 12 minutes it is treated as gone, and asked every 3 s until it answers |
| **P** Position | A coordinate set from a phone part-way through a session | Settings are re-read continuously; while no position is set the radio is also asked directly, every 60 s |
...
**How long silence is allowed to run, and why it is twelve minutes rather than eight.** Surviving one dropped reply means surviving until the report *after* the one that went missing, which is two whole intervals, not one — so the window has to clear ten minutes before it can be trusted at all. Widening it does not slow detection, because the loop no longer waits the window out in silence: at the halfway mark the radio is asked directly, every 30 s, so a genuinely dead radio is challenged at six minutes and confirmed dead sooner than the old eight-minute window managed. What the wider window buys is that a single lost reply on a quiet mesh can no longer put a false `radio` fault, and a false recovery after it, into a file of real field data.

**Silence is timed from the end of the self-test, not from the radio's last word.** The boot sequence can spend up to ten minutes waiting for a phone to hand over the time, and it spends that wait listening rather than asking. On a mesh with nothing else on it, the radio's last word is the reply it gave in the first few seconds — so the first recovery tick would open on a radio that had been "silent" for longer than the window and declare a perfectly good radio dead. The clock starts when the node starts logging. A radio that never answered at all is still tracked separately, so it is never told its radio just died.

**A probe is only fired when a probe can actually help.** Asking the radio for a report does nothing about a card fault — the card retries on its own timer — so an empty slot no longer triggers serial traffic every 30 s for the rest of the day. Each condition now sets its own rate: a silent radio every 3 s, a quiet-but-not-condemned radio every 30 s, a missing position every 60 s, and a missing card not at all.
...
- A recovery that happens while there is no card to write to is **held, not dropped**. A position arriving during a card outage changes what every row after it means, and the event fires exactly once; it is carried forward and written into the first status row of the file that eventually exists. Only the card's own return is skipped on a new file, because the `BOOT` row written a moment earlier already marks that instant.
...
**A block that recovers repeatedly is an error, not a warning.** One or two recoveries in a session is a node healing itself, which is the behaviour this layer exists to produce. Past `FLAPPING_RECOVERIES` of the same block, it has not recovered at all — it is oscillating, and every reading in the file sits either side of a boundary nobody can reason about. The checker reports it once, as `FLAPPING`, with a count and a span, rather than one line per event: a real bench run produced 105 identical `RECOVERED` warnings that buried the two errors underneath them. The cause there was a firmware timing bug (§9.4), and a file reported as clean-but-noisy is exactly how it stayed invisible.

**Recovery probes are not written to the card.** While something is wrong the radio is asked for a node report far more often than the ordinary five minutes. With forty-odd nodes known to a radio, logging every one of those replies would bury the packet rows the session exists to collect — so probe replies update the node's own state and are otherwise discarded. The five-minute schedule that *does* reach the card is untouched.

**`rows=` counts the file it appears in.** A new file starts its row count at zero. Carrying the count over from the card that was pulled out would produce a number that cannot be reconciled against anything — the reader has one file, and the count would be describing two. `drops=` deliberately stays cumulative for the opposite reason: it measures what the *session* lost, and a session spans however many files it took.

**A node no longer counts itself as a neighbour.** Packets can arrive before the radio has said what this node's own number is, and until it does, the "is this me?" test has nothing to compare against — so this node's own transmissions were being counted as a neighbour, both in the live count on the screen and in the `st_heard=` figure written into the `BOOT` row, where nobody reading the card later could correct it. The count is cleaned the moment the number is known, in both places.
...
9. Prove each check **recovers**, which is the other half of step 8 and the one that decides whether a box can be left alone (§9.4). Six separate runs, no power cycle in any of them:
    ...
    - Pull the card mid-session and put the **same** card back. The node must resume the same file — one `BOOT` row in it, not two — and the next status row must carry `recov=card` and a non-zero `drops=`. Confirm `rows=` in that status row counts only what this file holds.
    - Pull the card mid-session and put a **different** card in. A new file, again with its own `BOOT` row, and its first status row must show `rows=` restarted from zero rather than continuing the previous card's count.
    - Unplug the radio's serial line for fifteen minutes and reconnect it. The screen's `R` block and its verdict must both go bad, and both must come back; a status row must carry `recov=radio`. Fifteen minutes because the silence window is twelve — anything shorter proves nothing. Watch the console: probing must start at the six-minute mark, well before the block goes red.
    - Boot a node on a mesh with nothing else transmitting and let the clock wait run its full ten minutes, then let it log for twenty. The `R` block must stay green throughout and no `recov=radio` may ever appear — a false radio fault here is exactly what this window was widened to prevent.
    - Pull the card, then set the fixed position from a phone **while the slot is empty**, then push the card back. The first status row of the new file must carry `recov=pos` — the position change must not be lost just because there was nowhere to write it at the moment it happened.
